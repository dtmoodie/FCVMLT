...
static void buildParamLayout(QLayout* layout, QObject* parent, std::vector<parm>& parameters)
{
	...
	for (unsigned int i = 0; i < parameters.size(); ++i)
	{
		...
		if (parameters[i].type == t_char)
		{
			...
			connect(box, SIGNAL(valueChanged(QString)), parent, SLOT(paramChange(QString)));
		}
		...
		if (parameters[i].type == t_pullDown)
		{
			...
			parameters[i].box = box;
			connect(box, SIGNAL(currentIndexChanged(QString)), parent, SLOT(paramChange(QString)));
		}
	}
}*/
...
bool featureExtractor::extractFeatures(cv::Mat& src, featureContainer* features)
{
    if(type == sum)
    {
        ...
        {
			((float*)featureContainer->M.data)[i] = output[i];
        }
    }
    if(type == avg)
    {
    ...
    if(type == hist)
    {
        ...
        for(int i = 0; i < src.channels(); ++i)
        {
            for(int j = 0; j < histPlanes[i].rows; ++j)
            {
				...
				if (featureContainer->M.channels() == 3)
					featureContainer->M.at<cv::Vec3f>(0, j)[i] = val;
                else
					featureContainer->M.at<float>(0, j) = val;
            }
        }
    }
    if(type == sift)
    {
		...
		cv::drawKeypoints(src, keyPts, src, cv::Scalar(255, 0, 0));
...
featureContainer* featureWindow::extractFeatures(cv::Mat& src, cv::Mat& img)
{
	img = cv::Mat(); // Make empty image
	featureContainer* output = NULL;
    if(src.empty()) return output;
    if(method == wholeImage)
    {
        // Need to initialize the size of output inside this function so that it can be done once
		output = new featureContainer();
        output->M = cv::Mat::zeros(1,(*curExtractor)->numFeatures * src.channels(),CV_32F);
        (*curExtractor)->extractFeatures(src, output);
		if ((*curExtractor)->type == sift)
		{
			img = src;
		}
    }
...
featureContainer* featureWindow::extractFeatures(cv::Mat& src, cv::Mat& img, featureExtractor **curExtractor_)
{
    curExtractor = curExtractor_;
	return extractFeatures(src, img);
}
