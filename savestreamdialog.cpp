...
void saveStreamDialog::handleNewImage(container *cont)
{
    ...
    if(writer == NULL)
    {
		...
		opened = writer->open((ui->path->text() + ui->fileName->text()).toStdString(), fourCC, cont->frameRate, cont->img.size());
		if (!opened)
		{
			std::cout << "Codec not found" << std::endl;
			opened = writer->open((ui->path->text() + ui->fileName->text()).toStdString(), -1, cont->frameRate, cont->img.size());
		}
    }else
    {
        if(!opened)
